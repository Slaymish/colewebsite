...
export default function HeroSectionComponent({ section }: HeroSectionProps) {
  ...
  return (
    <section
      className={`relative flex p-5 md:p-8 ${bgUrl ? "text-white" : "bg-black/[0.03] text-black"}`}
      style={{
        minHeight,
    ...
    >
      ...
      {bgUrl && <div className="absolute inset-0 bg-black" style={{ opacity: overlayOpacity }} />}
      <div className="relative z-10 max-w-2xl" style={{ textAlign }}>
        {section.heading && (
          <h2 className="text-2xl font-bold tracking-[-0.04em] sm:text-4xl md:text-5xl">
            {section.heading}
          </h2>
        )}
        {section.subheading && <p className="mt-3 text-lg opacity-80">{section.subheading}</p>}
      </div>
    </section>
  );
}
